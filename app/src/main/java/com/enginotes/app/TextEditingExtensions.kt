...
// All the active-editor and committed-item-selection text logic — showTextSelectionBox,
// showInlineTextEditor, closeInlineEditor, and the smaller formatting/measurement helpers around
// them. This is the single biggest, most-edited cluster of code across this whole app's history:
// top-anchor positioning, pointer-tracking drag fixes, angle-based rotation, wrap-width settling,
// the fixed bottom toolbar. Pulling it into its own file (as extension functions on MainActivity,
// same technique as HatchExtensions.kt) means all of that lives in one place instead of being
// buried inside a 5000+ line file alongside completely unrelated features.
//
// A number of things this code touches (activeEditText, editingItem, editRotation, dp(), etc.)
// had to move from `private` to `internal` in MainActivity.kt to make this split possible —
// `internal` means "usable anywhere in this app module," not "exposed outside the app," so this
// doesn't change what the app exposes, just how its own files reach each other. Four pieces of
// actual mutable STATE (textSelectionBox, textSelectionItem, textSelectionHandles, layerToolbar)
// could NOT move here, even though the functions that use them did — an extension function has
// no way to add real stored properties to a class from outside it, so those four stay declared
// as real MainActivity properties, right at the top of the class.
...
internal fun MainActivity.toggleStyleOnSelection(et:EditText,styleFlag:Int){ val s=et.selectionStart;val e=et.selectionEnd; if(s==e){Toast.makeText(this,"Select text first",Toast.LENGTH_SHORT).show();return}; val from=minOf(s,e);val to=maxOf(s,e); val ed=et.text; val ex=ed.getSpans(from,to,StyleSpan::class.java).filter{it.style==styleFlag&&ed.getSpanStart(it)<=from&&ed.getSpanEnd(it)>=to}; if(ex.isNotEmpty()) for(sp in ex) ed.removeSpan(sp) else ed.setSpan(StyleSpan(styleFlag),from,to,Spannable.SPAN_EXCLUSIVE_EXCLUSIVE) }

internal fun MainActivity.applyColorToSelection(et:EditText,color:Int){ val s=et.selectionStart;val e=et.selectionEnd; if(s==e){editColor=color;et.setTextColor(color);return}; et.text.setSpan(ForegroundColorSpan(color),minOf(s,e),maxOf(s,e),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE) }
internal fun MainActivity.toggleUnderlineOnSelection(et:EditText){ val s=et.selectionStart;val e=et.selectionEnd; val from=minOf(s,e);val to=maxOf(s,e); val ed=et.text; val ex=ed.getSpans(from,to,UnderlineSpan::class.java).filter{ed.getSpanStart(it)<=from&&ed.getSpanEnd(it)>=to}; if(ex.isNotEmpty()) for(sp in ex) ed.removeSpan(sp) else ed.setSpan(UnderlineSpan(),from,to,Spannable.SPAN_EXCLUSIVE_EXCLUSIVE) }
    ...
    // actively typed in).
internal fun MainActivity.toggleFullItemSpan(item: TextItem, type: Char, value: Int) {
...
internal fun MainActivity.applyHighlightToSelection(et:EditText,color:Int){ et.text.setSpan(BackgroundColorSpan(color),minOf(et.selectionStart,et.selectionEnd),maxOf(et.selectionStart,et.selectionEnd),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE) }

internal fun MainActivity.dismissTextSelectionBox() {
...
internal fun MainActivity.measureTextBoxSize(item: TextItem, screenSizePx: Float): Pair<Int, Int> {
...
internal fun MainActivity.updateTextSelectionBoxSize(box: FrameLayout, moveSurface: View, item: TextItem): Pair<Int, Int> {
...
internal fun MainActivity.angleDegrees(pivotX: Float, pivotY: Float, touchX: Float, touchY: Float): Float =
        Math.toDegrees(kotlin.math.atan2((touchY - pivotY).toDouble(), (touchX - pivotX).toDouble())).toFloat()

internal fun MainActivity.showTextSelectionBox(item: TextItem, screenX: Float, screenY: Float, initialRawX: Float = -1f, initialRawY: Float = -1f) {
...
internal fun MainActivity.showInlineTextEditor(item: TextItem?, screenX: Float, screenY: Float, worldX: Float, worldY: Float) {
        dismissTextSelectionBox()
...
internal fun MainActivity.showEquationEditorDialog(initialLatex: String, allowDelete: Boolean, onConfirm: (String) -> Unit, onDelete: (() -> Unit)? = null) {
    ...
    val builder = AlertDialog.Builder(this)
        ...
        .setPositiveButton(if (initialLatex.isBlank()) "Insert" else "Update") { _, _ ->
            ...
            if (latex.isNotBlank()) onConfirm(latex)
        }
        .setNegativeButton("Cancel", null)
    if (allowDelete) builder.setNeutralButton("Remove") { _, _ -> onDelete?.invoke() }
    builder.show()
}
