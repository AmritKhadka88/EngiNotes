...
"))
    ...
    private fun showTableToolbar() {
        ...
        val isMerged = if (selStart != null && table != null)
            table.mergeSpans.containsKey(selStart) else false
        ...
        fun toolbarBtn(label: String, isActive: Boolean = false, action: () -> Unit) {
            ...
            b.setOnClickListener { action(); showTableToolbar() }
            toolbar.addView(b)
        }

        toolbarBtn("+ Row") {
            drawingView.addTableRow(selStart?.first ?: ((table?.rows ?: 1) - 1))
        }
        toolbarBtn("+ Col") {
            drawingView.addTableCol(selStart?.second ?: ((table?.cols ?: 1) - 1))
        }
        toolbarBtn("- Row") {
            drawingView.removeTableRow(selStart?.first ?: ((table?.rows ?: 1) - 1))
        }
        toolbarBtn("- Col") {
            drawingView.removeTableCol(selStart?.second ?: ((table?.cols ?: 1) - 1))
        }
        toolbarBtn("Merge", isActive = hasRange) {
            if (hasRange) drawingView.mergeCellSelection()
        }
        toolbarBtn("Unmerge", isActive = isMerged) {
            drawingView.unmergeCellSelection()
        }
        toolbarBtn("Style", isActive = selStart != null) {
            ...
            val r = (ss?.first ?: 0).coerceIn(0, t.rows - 1)
            val c = (ss?.second ?: 0).coerceIn(0, t.cols - 1)
        ...
        val dp2 = LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT)
        dp2.setMargins(dp(2), 0, dp(2), 0); doneBtn.layoutParams = dp2
    ...
    private fun showCellStyleDialog(table: TableItem, row: Int, col: Int, selEnd: Pair<Int, Int>?) {
        ...
        val minR = minOf(row, selEnd?.first ?: row)
        val maxR = maxOf(row, selEnd?.first ?: row).coerceIn(0, table.rows - 1)
        val minC = minOf(col, selEnd?.second ?: col)
        val maxC = maxOf(col, selEnd?.second ?: col).coerceIn(0, table.cols - 1)

        fun applyToSelection(action: (TableCell) -> Unit) {
            for (r in minR..maxR) for (c in minC..maxC) action(table.getCellPublic(r, c))
        ...
        fun label(text: String) {
            ...
            tv.setTextColor(Color.parseColor("#7B61FF"))
            tv.setPadding(0, dp(10), 0, dp(4)); container.addView(tv)
        }
...
Row") {
            for (c in 0 until table.cols) {
                ...
                hc.borderColor = Color.parseColor("#263238"); hc.borderWidth = 3f
            }
...
Borders") {
        ...
        textColorBtn.setOnClickListener { showColorGridDialog { c -> applyToSelection { it.textColor = c }; textColorBtn.setBackgroundColor(c) } }
        ...
        bgBtn.setOnClickListener { showColorGridDialog { c -> applyToSelection { it.bgColor = c }; bgBtn.setBackgroundColor(c) } }
        ...
        borderBtn.setOnClickListener { showColorGridDialog { c -> applyToSelection { it.borderColor = c }; borderBtn.setBackgroundColor(c) } }
        ...
        fun bwQuickBtn(lbl: String, weight: Float) {
            val b = Button(this); b.text = lbl; b.textSize = 12f
            b.setBackgroundColor(Color.parseColor("#EDE7F6")); b.setTextColor(Color.parseColor("#4527A0"))
            val p = LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT)
            p.setMargins(dp(2), 0, dp(2), 0); b.layoutParams = p
            b.setPadding(dp(8), dp(4), dp(8), dp(4)); b.minWidth = 0; b.minimumWidth = 0
            b.setOnClickListener { applyToSelection { it.borderWidth = weight }; bwSeek.progress = weight.toInt() }
            bwRow.addView(b)
        }
        ...
        bwSeek.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, v: Int, f: Boolean) { applyToSelection { it.borderWidth = v.toFloat() } }
            override fun onStartTrackingTouch(sb: SeekBar?) {}
            override fun onStopTrackingTouch(sb: SeekBar?) {}
        })
        ...
        tsSeek.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, v: Int, f: Boolean) { applyToSelection { it.textSize = v.toFloat().coerceAtLeast(8f) } }
            override fun onStartTrackingTouch(sb: SeekBar?) {}
            override fun onStopTrackingTouch(sb: SeekBar?) {}
        })
        ...
        for ((idx, lbl) in listOf("Left", "Center", "Right").withIndex()) {
            ...
            b.setOnClickListener { applyToSelection { it.alignment = idx } }
    ...
    private fun insertImage(uri: Uri) {
        try {
            ...
            val decodeOpts = android.graphics.BitmapFactory.Options().apply {
                inSampleSize = sample; inJustDecodeBounds = false; inPreferredConfig = Bitmap.Config.RGB_565
            }
