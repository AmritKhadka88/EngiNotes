...
class MainActivity : AppCompatActivity() {
    ...
    private val saveDocxLauncher = registerForActivityResult(ActivityResultContracts.CreateDocument("application/vnd.openxmlformats-officedocument.wordprocessingml.document")) { uri ->
        ...
        try {
            ...
            for(line in engFile.readLines()) if(line.startsWith("TEXT\u0001")){ val p=line.split("\u0001"); if(p.size>7){ val run=doc.createParagraph().createRun(); run.setText(p.last().replace("\u0002","\n")); try{ run.fontSize=(p[4].toFloat()/PT_TO_PX).toInt().coerceIn(8,72) }catch(e:Exception){} } }
    ...
    private fun setupBottomToolbar() {
        ...
        findViewById<Button>(R.id.btnDraw).setOnLongClickListener {
            showDrawPicker(it as Button); true
        }
    ...
    private fun toggleLayout() {
        if (isConvenientLayout) {
            // Switch to print — ask about rearranging text
    ...
    private fun setActiveTool(btn: Button?, tool: Tool, label: String) {
        drawingView.currentTool = tool; tvActiveTool.text = label; setActiveToolbarBtn(btn)
    }
    private fun setActiveToolbarBtn(btn: Button?) {
        activeToolbarButton?.isSelected = false; activeToolbarButton = btn; btn?.isSelected = true
    }
            ...
            when(item.title) {
                ...
                "Export Window" -> {
                    if(drawingView.canvasMode == CanvasMode.INFINITE) {
                        Toast.makeText(this,"Draw a rectangle to select",Toast.LENGTH_SHORT).show()
                        setActiveTool(null,Tool.EXPORT_WINDOW,"ExportWin")
                    } else Toast.makeText(this,"Switch to Infinite canvas for window export",Toast.LENGTH_SHORT).show()
                }
    ...
    private fun showSettingsDialog() {
        ...
        var selEraserIdx = if(drawingView.eraserMode==EraserMode.OBJECT) 0 else 1
        val eraserLbl = TextView(this).apply{ textSize=15f; setTextColor(Color.parseColor("#1565C0")); setPadding(0,dp(8),0,dp(8)) }
        fun refE(){ eraserLbl.text="Eraser: ${eraserTypes[selEraserIdx].substringBefore(" (")}  (tap)" }
        refE(); eraserLbl.setOnClickListener{ AlertDialog.Builder(this).setTitle("Eraser Type").setItems(eraserTypes){ _,i-> selEraserIdx=i; refE() }.show() }
        ...
        AlertDialog.Builder(this).setTitle("⚙ Settings").setView(scroll)
            .setPositiveButton("Done") { _,_ ->
                prefs.edit()
                    ...
                    .apply()
                drawingView.arcDivisions = prefs.getInt("arc_divisions",3)
                drawingView.eraserMode = if(selEraserIdx==0) EraserMode.OBJECT else EraserMode.AREA
                // Apply paper type to current note too
    ...
    private fun showAudioRecordDialog() {
        ...
        val dialog = AlertDialog.Builder(this).setTitle("Record Audio").setView(container)
            .setPositiveButton("⏺ Start") { _, _ -> /* overridden below */ }
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            if (!isRecording) {
                ...
                } else { Toast.makeText(this,"Could not start recording",Toast.LENGTH_SHORT).show() }
            } else {
                timerHandler?.removeCallbacksAndMessages(null)
                dialog.dismiss()
    ...
    private fun stopRecordingAndPlace(title: String = "Audio") {
        ...
        val item = AudioItem(file.absolutePath, drawingView.screenCenterWorldX(), drawingView.screenCenterWorldY(), title, durationMs)
        drawingView.addAudioItem(item)
    ...
    private fun launchChartFromTable(table: TableItem) {
        // Build series from table: first row = headers, first col = X labels
        ...
        // Pass table data as a simple CSV in an extra
        val sb = StringBuilder()
        for (r in 0 until rows) {
            val rowData = (0 until cols).map { c -> table.getCellPublic(r,c).text.trim() }
            sb.append(rowData.joinToString(",")); sb.append("\n")
        }
        intent.putExtra("table_csv", sb.toString())
    ...
    private fun showCellStyleDialog(table: TableItem, row: Int, col: Int, selEnd: Pair<Int,Int>?) {
        ...
        val div=View(this); div.layoutParams=LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,dp(1)).also{it.setMargins(0,dp(12),0,dp(4))}; div.setBackgroundColor(Color.LTGRAY); container.addView(div)
    ...
    private fun showTableCellEditor(table:TableItem,row:Int,col:Int,screenX:Float,screenY:Float) {
        ...
        val et=EditText(this).apply{
            ...
            addTextChangedListener(object:TextWatcher{ override fun beforeTextChanged(s:CharSequence?,start:Int,count:Int,after:Int){} override fun onTextChanged(s:CharSequence?,start:Int,before:Int,count:Int){} override fun afterTextChanged(s:Editable?){ cell.text=s?.toString()?:""; drawingView.invalidate() } })
        }
    ...
    private fun showInlineTextEditor(item: TextItem?, screenX: Float, screenY: Float, worldX: Float, worldY: Float) {
        ...
        tbtn(ptSize.toString()){btn-> AlertDialog.Builder(this).setTitle("Font Size (pt)").setItems((1..144).map{it.toString()}.toTypedArray()){ _,idx-> val pt=idx+1;editSize=pt*PT_TO_PX; val nss=if(useActualSize) editSize else editSize*drawingView.getScaleFactor(); et.textSize=(nss/density).coerceAtLeast(8f); btn.text=pt.toString(); drawingView.onScaleChanged?.invoke(drawingView.getScaleFactor()) }.show() }
